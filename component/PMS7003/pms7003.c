...
esp_err_t pms7003_activeMode(void)
{
    ...
    if (errorReturn == -1)
    {
        ESP_LOGE(__func__, "Send active command failed");
...
esp_err_t pms7003_readData(const int pms_modeAmbience, uint32_t *pm1_0, uint32_t *pm2_5, uint32_t *pm10)
{
    ...
    // Read raw data array
    int lenghtSensorDataArray = uart_read_bytes(CONFIG_PMS_UART_PORT, rawData, RX_BUFFER_SIZE, 100 / portTICK_RATE_MS);
    //xSemaphoreTake(print_muxtex, portMAX_DELAY);

    if (lenghtSensorDataArray >= 24 &&          // Can read data with only 24bytes in raw data array.
        rawData[0] == START_CHARACTER_1 &&      // Check first start byte in raw data array.
        rawData[1] == START_CHARACTER_2)        // Check second start byte in raw data array.
    {
        uint8_t startByte = (pms_modeAmbience == indoor) ? 4 : 10;          //atmospheric from 10th byte, standard from 4th in raw data array.
        ...
        *pm10  = PMS_ERROR_INVALID_VALUE;       // Return invalid value of sensor.
        ESP_LOGI(__func__, "PMS7003 sensor read data failed.");
        //xSemaphoreGive(print_muxtex);
        //vSemaphoreDelete(print_muxtex);
